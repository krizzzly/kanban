...
4. Worktree anlegen — **wie**, entscheidet `dockerStack` in `.claude/project.json`:

   Mit Stack (`true` oder fehlend):

   ```bash
   iwf worktree create NNNN <english_title>
   ```

   Ohne Stack (`false`) — reiner Git-Worktree vom ermittelten Basis-Branch:
