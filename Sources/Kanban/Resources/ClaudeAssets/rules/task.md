...
- **Und dieselbe Sache zusätzlich nach `## JIRA Lösungsfeld` → `### Für Kunde`** in den Block „Entscheidungen, Abweichungen, Annahmen": jede Entscheidung, jede Abweichung von den Akzeptanzkriterien, jede Annahme, alles bewusst Ausgelassene. Grund: **das Task-File wird nicht mitcommittet** — nur dieser Unterabschnitt wandert ins Jira-Feld „Lösung" und ist nach dem Merge die einzige Stelle, an der es noch steht. `## Entscheidungen` ist die Arbeitsspur, `### Für Kunde` das, was überlebt
- Was NICHT rein:
